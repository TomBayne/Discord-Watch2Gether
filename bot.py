...
async def on_message(message):
    ...
    if message.content.startswith('%status'): #if a user types %status
        await message.channel.send('Watch2gether bot is currently running and accepting commands. Please contact tomba#4032 if you are having issues.')
        await message.channel.send('Serving '+ str(len(client.users)) +' users across '+ str(len(client.guilds)) + ' servers!')
    ...
    if message.content.startswith('%about'): #if a user types %about
        await message.channel.send("Thanks for installing Watch2Gether bot! This bot is not affiliated with the Watch2Gether website.\nCheck bot status using '%status'.\nReach this page with '%about'.\nCreate a new Watch2Gether room using '%createroom'.\nBot is currently running Version 1.0.0. Please contact tomba#4032 if you have any issues with this bot.")
    ...
    if message.content.startswith('%help'): #if a user types %help
        await message.channel.send("Create a new room by typing '%createroom'\nCheck bot service status by typing '%status'\nRead more about the bot by typing '%about'")
        print(nowNice,message.author, ' requested bot about page on ', message.guild.name) #log to console
        return
    
   
    if message.content.startswith('%createroom'):
       print(nowNice,message.author,' requested a new room to be created. (', message.guild.name,')') #log to console
       headers = {
       'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8',
       'Content-Type': 'application/x-www-form-urlencoded',
       }
       response = requests.post('https://w2g.tv/rooms/create', headers=headers) #send request
       print(nowNice, 'Generated URL for ', message.author, 'in guild: ', message.guild.name,'(', response.url, ')') #log to console
       ...
       embedVar = discord.Embed(title="Watch2Gether Room is Ready!", description=descriptionText, color=0x00ff00)#create embed
